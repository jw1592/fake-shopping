...
module.exports = async (req, res) => {
  ...
  try {
      ...
      return new Promise((resolve) => {
        req.on('end', async () => {
          try {
            ...
            } else if (finalImages[0] && !finalImages[0].includes('placeholder')) {
            ...
            console.log('Page stored with ID:', id);
    ...
    if (productPageMatch && method === 'GET') {
      ...
      console.log('Product page request for ID:', id, 'Found:', !!data);
      
      if (!data) {
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        return res.status(404).send('<h1>404 - 페이지가 존재하지 않습니다.</h1><p><a href="/">← 메인 페이지로 돌아가기</a></p>');
      }
